...
import { Table, Form, Input, Icon, Button, Drawer, Pagination, Alert, Modal, Spin } from 'antd'
...
export class Inquire extends Component {
	...
	getTableData(nowPage = 1) {
		...
		getInquireList(nowPage, pageSize)
			.then((res) => {
				this.setState({ data: res.list.inquire, addList: res.list.allCount, spinning : false })
			})
	}
	fullScreen = () => {
	...
	render() {
		let { columns, data, Stainquire, nowPage, visible, addList, rowSelection, selectLength, sum, madalvisible, updataInfo, spinning } = this.state
		return (
			<div>
				<Form layout="inline">
					<Form.Item>
